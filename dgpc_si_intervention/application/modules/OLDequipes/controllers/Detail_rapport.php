    ...
    if ($donne[0]=='DGPC') {
      if ($donne[1]=='Materiels') {
        ...
        foreach ($degat_dgpc as $row) {
            ...
            // $mater=$this->Model->getOne('interv_materiaux',array('MATERIEL_CODE'=>MATERIEL_ENDO_CODE))""
            $cat=$this->Model->getOne('tk_causes',array('CAUSE_ID'=>$mat['CAUSE_ID']));
            $sub_array[] = $cat['CAUSE_DESCR'];
            $cppc=$this->Model->getOne('rh_cppc',array('CPPC_ID'=>$mat['CPPC_ID']));
