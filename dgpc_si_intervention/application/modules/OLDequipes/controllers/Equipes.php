...
$template=array(
        ...
        if($this->mylibrary->verify_is_admin() ==1){
          $this->table->set_heading('NOM','TELEPHONE','EMAIL','CPPCs','Service','STATUT','MEMBRE','OPTIONS');
        }else{
          $this->table->set_heading('NOM','TELEPHONE','EMAIL','CPPCs','Service','STATUT','MEMBRE');
        }
...
$equipe_id = $this->uri->segment(4);
    ...
    public function supprimer()
    {
      ...
      $equipe = $this->Model->getOne('interv_equipe',array('EQUIPE_ID'=>$equipe_id));
      $interventions = $this->Model->getList('interv_intervention',array('EQUIPE_ID'=>$equipe_id));
      
      $msg = "<font color='red'>L'équipe cherchée n'est plus.</font>";
      if(!empty($equipe)){
        if(empty($interventions)){
           $this->Model->delete('interv_equipe',array('EQUIPE_ID'=>$equipe_id));
