  ...
  public function autho()
    ...
    public function save()
    {
       ...
       $info_cause=$this->Model->getOne('tk_causes',array('CAUSE_ID'=>$this->input->post('CAUSE_ID')));
        ...
        if ($this->form_validation->run() == FALSE) {
              ...
              if($check==1){
               ...
               $this->load->view('ticket/Ticket_Nouveau_View',$data); 
                ...
                $array_ticket = array(
                                ...
                                'TICKET_DECLARANT'=>ucfirst($this->input->post('TICKET_DECLARANT')),
                                'CAUSE_ID'=>$cause_id,
            ...
            if($ticket_id >0){
             ...
             //$msg = "<font color='green'> Cet évenement <b>".$this->input->post('TICKET_DESCR')."</b> a été enregistré.</font>";
              ...
              $notificat=$this->notification($id_notif_caserne,$localite_notif,$this->input->post('TICKET_DESCR'),$tk_code_notif);
            ...
            $notificat=$this->notification($id_notif_caserne,$localite_notif,$code_tkt,$this->input->post('TICKET_DESCR'),$NOTIFIE_DAHMI);
        ...
        public function notification($cppc_id,$localite_notif,$tk_code,$ticket_desc,$NOTIFIE_DAHMI){
        $statut_notif = 0;
        ...
        //Notifier Manager Equipe 
           ...
           if(!empty($personnel_chef)){
              $msg_manager = "Cher(e), ".$personnel_chef['PERSONNEL_NOM']." ".$personnel_chef['PERSONNEL_PRENOM']." nouvelle intervention :<br> CODE: ".$tk_code."<br> Lieu : ".$localite_notif."<br> Description: ".$ticket_desc;

              $msg_manager_tel = "Cher(e), ".$personnel_chef['PERSONNEL_NOM']." ".$personnel_chef['PERSONNEL_PRENOM']." nouvelle intervention :<br> CODE: ".$tk_code."<br> Lieu : ".$localite_notif."<br> Description: ".$ticket_desc;

              $subjet = "Appel à une intervention";
              $this->notifications->smtp_mail($personnel_chef['PERSONNEL_EMAIL'],$subjet,NULL,$msg_manager,NULL);
              $this->notifications->send_sms('+257'.$personnel_chef['PERSONNEL_TELEPHONE'],$msg_manager_tel);
            }
        ...
        if($this->input->post('CAUSE_ID') == 14){
           ...
           foreach ($membre_dhami as $membre) {           
              $msg_dhami = "Cher(e), ".$membre['PERSONNEL_NOM']." ".$membre['PERSONNEL_PRENOM']." nouvelle intervention :<br> CODE: ".$tk_code."<br> Lieu : ".$localite_notif."<br> Description: ".$ticket_desc;

              $msg_dhami_tel = "Cher(e), ".$membre['PERSONNEL_NOM']." ".$membre['PERSONNEL_PRENOM']." nouvelle intervention : CODE: ".$tk_code." Lieu : ".$localite_notif." Description: ".$ticket_desc;

             $this->notifications->smtp_mail($membre['PERSONNEL_EMAIL'],$subjet,NULL,$msg_dhami,NULL);
             $this->notifications->send_sms('+257'.$membre['PERSONNEL_TELEPHONE'],$msg_dhami_tel);
           }
        }
        ...
        if(!empty($equipe_sur_horaire)){
          ...
          foreach($membres as $membre){
            ...
            $msg_intervention = "Cher(e), ".$collaborateur['PERSONNEL_NOM']." ".$collaborateur['PERSONNEL_PRENOM']." nouvelle intervention :<br> CODE: ".$tk_code."<br> Lieu : ".$localite_notif."<br> Description: ".$ticket_desc;

            $msg_intervention_tel = "Cher(e), ".$collaborateur['PERSONNEL_NOM']." ".$collaborateur['PERSONNEL_PRENOM']." nouvelle intervention : CODE: ".$tk_code." Lieu : ".$localite_notif." Description: ".$ticket_desc;
           ...
           $array_notification = array(
            ...
            $this->notifications->send_sms('+257'.$collaborateur['PERSONNEL_TELEPHONE'],$msg_intervention_tel);
    ...
    public function notification_appui($cppc_id,$localite_notif,$tk_code,$ticket_desc,$appuis){
       ...
       if(!empty($equipe_sur_horaire)){
          ...
          foreach($membres as $membre){
            ...
            $msg_intervention = "Cher(e), ".$collaborateur['PERSONNEL_NOM']." ".$collaborateur['PERSONNEL_PRENOM']." nouvelle intervention :<br> CODE: ".$tk_code."<br> Lieu : ".$localite_notif."<br> Description: ".$ticket_desc;
            $msg_intervention_tel = "Cher(e), ".$collaborateur['PERSONNEL_NOM']." ".$collaborateur['PERSONNEL_PRENOM']." nouvelle intervention : CODE: ".$tk_code."<br> Lieu : ".$localite_notif." Description: ".$ticket_desc;
    ...
    // TROUVER MATERIEL ET EQUIPE
     $id=$id_cppc;
      ...
      $result="<h4>Liste des équipes et effectifs disponibles</h4><table class='table table-bordered  table-responsive'><th>EQUIPE</th><th>EFFECTIF</th>";
      $results="";
      foreach ($equipe as $key) {
       ...
       $mb="<h4>Liste des membre de l'equipe ".$key['EQUIPE_NOM']."</h4>";
        $i=1;
foreach ($membre as $val) {
	if ($val['IS_CHEF_EQUIPE']==1) {
		$chef="oui";
	}else{ $chef="non";}
  $personnel=$this->Model->getOne('rh_personnel_dgpc',array('PERSONNEL_ID'=>$val['PERSONEL_ID']));
    $mb.="<tr><td>".$personnel['PERSONNEL_NOM']."</td><td>".$personnel['PERSONNEL_PRENOM']."</td><td>".$personnel['PERSONNEL_TELEPHONE']."</td><td>".$personnel['FONCTION']."</td><td>".$personnel['GRADE']."</td><td>".$chef."</td></tr>";

    // $mb.=" $i. ".$personnel['PERSONNEL_NOM']." ".$personnel['PERSONNEL_PRENOM']." ".$personnel['PERSONNEL_TELEPHONE']." <p>";
     ...
     $i++;
                                        ...
                                        <div class='modal-dialog'>
                                            <div class='modal-content'>
                                                ...
                                                <div class='modal-body'>
                                                 <table id='materielCat' class='materielCat table table-bordered  table-responsive'><thead><tr><th>NOM</th><th>PRENOM</th><th>TELEPHONE</th><th>FONCTION</th><th>GRADE</th><th>Est-il chef?</th></tr></thead><tbody>
                                                    " .$mb . "</tbody>
                                                    </table>
      ...
      $result.="</table><h4>Liste des matériels disponibles</h4><table class='table table-bordered  table-responsive'><th>MATERIEL</th><th>CATEGORIE</th><th>ETAT</th>";
      ...
      public function liste()
    {
        ...
        $critere_array = array();
        $critere_array['tkt.MODE_GET_INFO'] = 0;
      ...
      $degat_mat=$this->Model->getList('interv_odk_degat_materiel',array('TICKET_CODE'=> $row->TICKET_CODE));
        $degat_mort=$this->Model->getList('interv_odk_degat_humain',array('TICKET_CODE'=> $row->TICKET_CODE,'STATUT_SANTE'=>1));
      $degat_blesse=$this->Model->getList('interv_odk_degat_humain',array('TICKET_CODE'=> $row->TICKET_CODE,'STATUT_SANTE'=>0));

            
            $sub_array[] = $row->TICKET_DESCR;
            $sub_array[] = $row->CAUSE_DESCR;
            $sub_array[] = "-".sizeof($degat_mort)."Mort(s) <br>"."-".sizeof($degat_blesse)."Blessé(s)";
        ...
        $this->table->set_heading('CODE INTERV.','OUVERT A','FERME A','DESCRIPTION', 'CAUSE','DEGATS HUMAINS', 'LOCALITE','DECLARANT', 'CPPC', 'STATUT','CHEF EQUIPE','OPTIONS');
   ...
   $degat_dgpc=$this->Model->getList('interv_odk_degat_materiel',array('TICKET_CODE'=>$ticket_code,'CONCERNE_DGPC'=>1));
     ...
     $data['series'] =$serie;
      ...
      $data['title'] = "Rapport d'intervention chef equipe";
      $data['breadcrumb'] = $this->make_bread->output();
    ...
    public function infoCause(){
     ...
     if ($cause['CAUSE_CODE']=="AUTR"){
      ...
      if(isset($_POST['CAUSE_ID'])){ 
        ...
        $titre="Liste des matériels disponibles pour <b>".$cause['CAUSE_DESCR']."</b>";
         }else{
          $CAUSE_ID=0;
          $titre="Liste des matériels relatifs</b>";
         }
      

      // $mat=$this->Model->getListOrder('interv_materiaux',array('CPPC_ID'=>$id,'CATEGORIE_ID'=>$CATEGORIE_ID),'MATERIEL_DESCR');
         $cppc_mat=$this->Model->getOne('rh_cppc',array('CPPC_ID'=>$id));
                                    ...
                                    <div class='modal fade' id='mydelete" .$key['EQUIPE_ID']. "'>
                                        <div class='modal-dialog'>
                                            <div class='modal-content'>
                                                ...
                                                <div class='modal-body'>
                                                 <table id='materielCat' class='materielCat table table-bordered  table-responsive'><thead><tr><th>NOM</th><th>PRENOM</th><th>TELEPHONE</th><th>FONCTIN</th><th>GRADE</th><th>Est-il chef?</th></tr></thead><tbody>
                                                    " .$mb . "</tbody>
                                                    </table>
...
foreach ($mat1 as $value) {
        ...
        $result1.="<tr><td>".$value['MATERIEL_DESCR']."</td><td>".$cause['CAUSE_DESCR']."</td><td>".$etat['DESCRIPTION']."</td></tr>";
      }
      if (!$mat1) {
      	$result1.="<tr><td colspan='3'><center style='color:red'>Aucun matériel disponible</center></td></tr>";
      }
      $result.="</table>
 ...
 <div class='modal fade' id='plus_materiel'>
      ...
      if (sizeof($mat)==0) {
        $result.="<tr><td colspan='3'><center style='color:red'>Aucun matériel pour <b>".$cause['CAUSE_DESCR']."</b> disponible ".$cppc_mat['CPPC_NOM']."</center></td></tr>";
      }
      $result.="</table>";
      $result.="</table><a href='' id='' data-toggle='modal' data-target='#plus_materiel' class='btn btn-success'>Tous les matériels disponibles</a>";
...
//DEGA HUMAIN
      ...
      if ($ticket['STATUT_ID']==5) {
        $data['cloture_tiquet'] =$ticket['DATE_INSERTION'];
      }else
...
$serie_materiel="";
    ...
    //---------RAPPORT CPPC CYCLE INTERENTION2-----------------
      ...
      // $m=explode(":", $h[1]);
      // $min=$m[0];
      $date_ouverture = new DateTime($ticket['DATE_INSERTION']);
      if ($ticket['DATE_INSERTION']) {
        $ouv=$ticket['DATE_INSERTION'];
        // $ouv_val=10;
        $ouv_val=5;
      }else{
        $ouv="NULL DATE";
       ...
       $date_intervention = new DateTime($ticket['DATE_INTERVENTION']);
      if ($ticket['DATE_INTERVENTION']) {
        $interv=$ticket['DATE_INTERVENTION'];
        // // $ouv_val=10;
        // $interv_val=5;
        if($date_intervention>$date_ouverture){
      $diff_interv = $date_intervention->diff($date_ouverture);

      // $dure= $diff->format('%a jours %h heures %i minutes %s secondes');
      $dure_interv= $diff_interv->format('%a')*24*60;
      $dure_interv1= $diff_interv->format('%h')*60;
      $dure_interv2= $diff_interv->format('%i');
      // echo $dure1;exit();
        $interv_val=$dure_interv+$dure_interv1+$dure_interv2;
      }else{$interv_val=0;}
      }else{
        $interv="NULL DATE";
         $interv_val=0;
...
if($date_arrive>$date_ouverture){
 ...
 $data['ouverture']="( ".$ouv.") ";
      ...
      $data['intervention_val']=$interv_val;
      $data['sortie']="(".$sort.")";
      $data['sortie_val']=$sortie_val;
      $data['arriver']="(".$arri.")";
      $data['arriver_val']=$arri_val;
 ...
 $cppc_mat=$this->Model->getOne('rh_cppc',array('CPPC_ID'=>$id));
      ...
      foreach ($cppc as $ke) {
       ...
       $ap.="</table";
$results="";
      $result="<h4>Liste des équipes et effectifs disponibles </h4>
                                    ...
                                    <div class='modal fade' id='mydelete" .$key['EQUIPE_ID']. "'>
                                        <div class='modal-dialog'>
                                            <div class='modal-content'>
                                                ...
                                                <div class='modal-body'>
                                                 <table id='materielCat' class='materielCat table table-bordered  table-responsive'><thead><tr><th>NOM</th><th>PRENOM</th><th>TELEPHONE</th><th>FONTION</th><th>GRADE</th><th>Est-il chef?</th></tr></thead><tbody>
                                                    " .$mb . "</tbody>
                                                    </table>
...
foreach ($mat1 as $value) {
      ...
      <h4>Liste des matériels relatifs</h4>
 ...
 <div class='modal fade' id='plus_materiel'>
      ...
      $result.="</table><a href='' id='' data-toggle='modal' data-target='#plus_materiel' class='btn btn-success'>Tous les materiels disponibles</a>";
      echo $result.$results;
    }
