 ...
 class Moyenne_temps extends MY_Controller
 {
  ...
  public function index(){
      ...
      $data['cppc']= $this->Model->getList('rh_cppc',array());
  	  ...
  	  $max_mois = $this->Model->getRequete('SELECT MAX(DATE_FORMAT(DATE_INTERVENTION,"%m")) as max_month FROM tk_ticket WHERE DATE_INTERVENTION LIKE "%'.$annee.'%"  order by max_month DESC');
      $mois_max=0;
      foreach ($max_mois as $key => $value)
       {
  	  ...
  	  if($mois_max < 4){
         $data['trim_s']= '1';  
  	  }else if($mois_max < 7 && $mois_max > 3){
          $data['trim_s']= '2';
  	  }else if($mois_max < 10 && $mois_max > 6){
          $data['trim_s']= '3';
  	  }else if($mois_max > 9){
          $data['trim_s']= '4';
  	  }
      ...
      $data['title']="Moyenne d'intervantion";
