    ...
    function index(){
      ...
      if(!empty($liste_realisation)){
        ...
        foreach ($liste_realisation as $key => $info_inc) {
            ...
            $icon=$url.'leaflet/icons/arch.png';
            $TICKET_DECLARANT = $info_inc['TICKET_DECLARANT'];
            $titre='TICKET';
                ...
                if($etat==1){
                  ...
                  $tab_Infos = $tab_Infos . $info_inc['CPPC_ID'] . '<>' . str_replace("'", "", $info_inc['CPPC_DESCR']) . '<>' . $info_inc['LATITUDE'] . '<>' . $info_inc['LONGITUDE'] . '<>' . $MarkerIcon . '<>' . $MarkerFixe .'<>'.$icon.'<>'.$compteur.'<>'.$TICKET_DECLARANT.'<>'.$titre.'<>'.'#';
                ...
                if($etat==3){
                  $tab_Infos = $tab_Infos . $info_inc['TICKET_ID'] . '<>' . str_replace("'", "", $info_inc['TICKET_DESCR']) . '<>' . $info_inc['LATITUDE'] . '<>' . $info_inc['LONGITUDE'] . '<>' . $MarkerIcon . '<>' . $MarkerFixe .'<>'.$compteur.'<>'.$TICKET_DECLARANT.'<>'.$titre.'<>'.'#';
                }
