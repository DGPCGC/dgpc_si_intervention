<?php
 ...
 class Notification_Alert extends MY_Controller
 {
  ...
  {
    parent::__construct();
    $this->make_bread->add('Notifications Alert', "alerte/Notification_Alert", 0);
...
function select_personne()
  { 
    ...
    foreach ($personne as $value) {
     $rst.="<option value='".$value["PERSONNEL_ID"]."'>".$value["PERSONNEL_NOM"].' '.$value['PERSONNEL_PRENOM']."</option>";
    }
    if($rst!=""){
    echo $rst;
  ...
  public function select_partenaire()
  {
    ...
    foreach ($partenaires as $value) {
     $liste_partenaires.="<option value='".$value["PARTENAIRE_ID"]."'>".$value["PARTENAIRE_DESCR"]."</option>";
    }
  ...
  function select_institution(){
    ...
    foreach ($institution as $value) {
     $rst.="<option value='".$value["INSTITUTION_ID"]."'>".$value["NOM_INSTITUTION"]."</option>";
    }
    if($rst!=""){
    echo $rst;
    }else echo "<option value='0'>Aucun</option>";
          ...
          foreach ($categorie as $key => $value):
            ...
            $message_id=$this->Model->insert_last_id($table,$datas);
                        ...
                        if(empty($sms)){
                           ...
                           $this->notifications->send_mail($infos_perso['PERSONNEL_EMAIL'],'DGPC - Personnel',array(),$message_to_send,array());
                           $tt = $this->notifications->send_sms('+257'.$infos_perso['PERSONNEL_TELEPHONE'], $message_to_send);
            ...
            $destina = $this->Model->getList('notif_institution',array('INSTITUTION_ID' => $institution[$i]));
                        ...
                        if(empty($sms)){
                           $message_to_send = $this->input->post('MESSAGE');
                           $this->notifications->send_mail($infos_inst['EMAIL'],'DGPC - test - Institution',array(),$message_to_send,array());
                           $tt = $this->notifications->send_sms('+257'.$infos_inst['TELEPHONE'], $message_to_send);
                           echo $tt;
                        }else if(empty($email)){
                           $message_to_send = $this->input->post('MESSAGE');
                           $this->notifications->send_mail($infos_inst['EMAIL'],'DGPC - test - Institution',array(),$message_to_send,array());
            ...
            foreach ($partenaire as $key => $value) {
              ...
              $CHECK=$this->input->post('CHECK1[]');
                        ...
                        if(empty($sms)){
                           ...
                           $retr = $this->notifications->send_sms('+257'.$infos_part['PARTENAIRE_TEL'], $message_to_send);
 ...
 public function liste()
  ...
  if($personne){
    $alert_notif[]="<b>Personnel : </b>".$personne['PERSONNEL_NOM'].' '.$personne['PERSONNEL_PRENOM'];
...
$tableaudata[]=$alert_notif;
      ...
      $template = array(
                ...
                $this->table->set_heading(array('Objet','Message','Categories','Téléphone','Email','Date'));
                $data['alert_notif']=$tableaudata;
                $data['title']="Liste des alertes notifiés";
                $this->load->view('notification_view/Alerte_Notification_List_View',$data);
    }

    public function message()
    {
      $message = "Message de test un";
      $request = $this->notifications->send_sms('+25769710958', $message);
