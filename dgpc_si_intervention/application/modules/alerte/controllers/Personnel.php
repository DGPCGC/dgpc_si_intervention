...
class Personnel extends CI_Controller {
    ...
    public function add()
  {
    ...
    'PERSONNEL_NOM'=>strtoupper($this->input->post('NON')),
    ...
    $check=$this->Model->checkvalue('notif_personnel',array('PERSONNEL_TELEPHONE'=> $this->input->post('PHONE')));
    if ($check!=1) {
      $this->Model->create('notif_personnel',$datas);
    $data['message']='<div class="alert alert-success text-center">'."L'enregistrement de <b>".$this->input->post('NON').' - '.$this->input->post('PRENOM'). '</b> faite avec succès</div>';
      ...
      $data['message']='<div class="alert alert-danger text-center">Ce numero de téléphone existe deja</div>';
    }
  ...
  public function listing(){
    ...
    $data["table"] ="<table id='mytable' class='table table-responsive'><thead><tr><th>Agent </th><th>Commune </th><th>Colline</th><th>Télephone </th><th>Action</th></tr></thead><tbody>";
    foreach ($personnel as $value) {
    
    $comm=  $this->Model->getOne('ststm_communes',array('COMMUNE_ID' =>$value['COMMUNE_ID']));
    
   
    $data["table"] .= '<tr><td>'.$value["PERSONNEL_NOM"].' - '.$value["PERSONNEL_PRENOM"].'</td><td>'.$comm["COMMUNE_NAME"].'</td><td>'.$value["COLLINE"].'</td><td>'.$value["PERSONNEL_TELEPHONE"].'</td>
              <td>
                  <div class="dropdown ">
                   ...
                   <li><a data-toggle="modal" data-target="#mydetail'.$value["PERSONNEL_ID"].'">Detail</a></li>
                     ...
                     <li> <a href='.base_url("alerte/Personnel/getOne/").$value["PERSONNEL_ID"].'>Modifier</a></li> 
      ...
      $data['title'] = "Liste des personnels notifiés DGPC";
...
function getOne(){
      ...
      $com=$this->Model->getOne('ststm_communes',array('COMMUNE_ID'=>$personnels['COMMUNE_ID']));

 
      $data['personnel']=$personnels;
      $data['prov']=$com['PROVINCE_ID'];
  ...
  function update()
{
      ...
      $sav=$this->Model->update('notif_personnel',array('PERSONNEL_ID'=>$idn),$datas);
      $data['message']='<div class="alert alert-success text-center">'."Modification de ".$this->input->post('NON').' - '.$this->input->post('PRENOM'). ' faite avec succès"</div>';
      $this->session->set_flashdata($data);
