...
*/
    ...
    if($PROVINCE_ID>0){
      $criteresnew['PROVINCE_ID']=$PROVINCE_ID;
      ...
      $tab_coordonne=$provice['PROVINCE_LATITUDE'].','.$provice['PROVINCE_LONGITUDE'];
       $center=12;
       $rh_cppc=$this->Model->getList('rh_cppc',$criteresnew);
    ...
    if($CPPC_ID>0){
      ...
      $rh_cppc=$this->Model->getList('rh_cppc',array());
      $tk_tickets=$this->Model->getList('tk_ticket',array('CPPC_ID'=>$CPPC_ID));
       
    }

    if(!empty($datecreation)){
      $criteresnew['DATE(DATE_INSERTION)']=$datecreation;
    }

    $caserne = $this->Model->getList('rh_cppc',$criteresnew);

    if($TICKET_ID>0){
       $tk_ticketone=$this->Model->getOne('tk_ticket',array('TICKET_ID'=>$TICKET_ID));
       $tab_coordonne=$tk_ticketone['LATITUDE'].','.$tk_ticketone['LONGITUDE'];
       $center=13;
